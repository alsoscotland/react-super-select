...
var ReactSuperSelect = React.createClass({
  ...
  _findArrayOfOptionDataObjectsByValue: function _findArrayOfOptionDataObjectsByValue(value) {
    ...
    return _.reject(this.state.data, function (item) {
      return !_.includes(valuesArray, item[_this.state.valueKey]);
    });
  },
  ...
  _getNormalDisplayMarkup: function _getNormalDisplayMarkup() {
    ...
    return _.map(this.state.value, function (value) {
      var selectedKey = "r_ss_selected_" + value[_this2.state.labelKey];
      if (_this2.props.customOptionTemplateFunction) {
        return _this2.props.customOptionTemplateFunction(value);
      } else {
        return React.createElement(
          'span',
          { key: selectedKey, className: 'r-ss-selected-label' },
          value[_this2.state.labelKey]
        );
      }
    });
  },
  ...
  _getOptionsMarkup: function _getOptionsMarkup() {
    ...
    if (!_.isArray(data)) {
      _.forIn(data, function (groupedOptions, heading) {
        options.push(_this3._getGroupHeadingMarkup(heading));
        options = options.concat(_this3._getTemplatedOptions(groupedOptions, optionsCount));
  ...
  _getTagsDisplayMarkup: function _getTagsDisplayMarkup() {
    ...
    return _.map(this.state.value, function (value) {
      return _this4._getTagMarkup(value);
    });
  },
  ...
  _mapDataToOptionsMarkup: function _mapDataToOptionsMarkup(data, indexStart) {
    ...
    return _.map(data, function (dataOption, index) {
      ...
      var isCurrentlySelected = _this5._isCurrentlySelected(dataOption),
          itemKey = "drop_li_" + dataOption[_this5.state.valueKey],
          indexRef = 'option_' + index,
          ariaDescendantId = _this5.state.controlId + '_aria_' + indexRef,
          optionMarkup = _.isFunction(_this5.props.customOptionTemplateFunction) ? _this5.props.customOptionTemplateFunction(dataOption) : dataOption[_this5.state.labelKey],
      ...
      return React.createElement(
        ...
        { ref: indexRef,
          ...
          key: itemKey,
          'data-option-value': dataOption[_this5.state.valueKey],
          onClick: _this5._selectItemOnOptionClick.bind(null, dataOption),
          role: 'option' },
        optionMarkup
      );
    });
  ...
  _removeAllOptionsInOptionIdRange: function _removeAllOptionsInOptionIdRange(from, to) {
    ...
    var remainingSelected = _.reject(this.state.value, function (opt) {
      return _.includes(valuePropsToReject, opt[_this6.state.valueKey]);
    });
  ...
  _removeSelectedOptionByValue: function _removeSelectedOptionByValue(value, callback) {
    ...
    var SelectedAfterRemoval = _.reject(this.state.value, function (option) {
      return option[_this7.state.valueKey] === value[_this7.state.valueKey];
    });
  ...
  _selectAllOptionsInOptionIdRange: function _selectAllOptionsInOptionIdRange(from, to) {
    ...
    var optionsToSelect = _.reduce(this.state.data, function (memo, opt) {
      if (_.includes(valuePropsToSelect, opt[_this8.state.valueKey])) {
        memo.push(opt);
      }
      return memo;
    }, []);
